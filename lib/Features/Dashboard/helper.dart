...
Future<bool> checkSubscription(BuildContext context) async {
  try {
    ...
    final isSubscribed = subscribedValue == true ||
        subscribedValue.toString().toUpperCase() ==
            'TRUE'; // ✅ handles text "TRUE"

    if (!isSubscribed) {
      // This awaits the dialog so the caller does not continue and accidentally dismiss it immediately.
      if (context.mounted) await _showNotSubscribedDialog(context);
      return false;
    }
    ...
    if (endDate == null || !DateTime.now().isBefore(endDate)) {
      ...
      // This awaits the dialog so the caller does not continue and accidentally dismiss it immediately.
      if (context.mounted) await _showExpiredDialog(context);
      return false;
    }
...
// This dialog informs the institution that the requested feature requires an active subscription.
Future<void> _showNotSubscribedDialog(BuildContext context) async {
...
// This dialog informs the institution that its subscription is expired and must be renewed.
